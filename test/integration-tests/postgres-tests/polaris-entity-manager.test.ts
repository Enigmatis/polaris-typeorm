...
describe('entity manager tests', () => {
    ...
    describe('soft delete tests', () => {
        it('parent is not common model, hard delete parent entity', async () => {
            ...
            const lib = await connection.manager.findOne(Library, new PolarisFindOneOptions(criteria, generateContext()) as any);
            expect(lib).toBeDefined();
            await connection.manager.delete(Library, new PolarisCriteria(criteria, generateContext()));
            const libAfterDelete = await connection.manager.findOne(Library, new PolarisFindOneOptions(criteria, generateContext()) as any);
            expect(libAfterDelete).toBeUndefined();
        });

        it('field is not common model, does not delete linked entity', async () => {
            await connection.manager.delete(Author, new PolarisCriteria(authorWithCascadeCriteria, generateContext() as any));
            const lib = await connection.manager.findOne(Library, new PolarisFindOneOptions({relations: ['books']}, generateContext()) as any);
            const criteria = {
                where: {
            ...
            const authorWithCascade = await connection.manager.findOne(Author, new PolarisFindOneOptions(criteria, generateContext()) as any);
            expect(lib).toBeDefined();
            authorWithCascade
        ...
        it('parent and field are common models but cascade is not on, does not delete linked entity', async () => {
            ...
            await connection.manager.delete(User, new PolarisCriteria(userCriteria, generateContext()));
            const userCommonModel = await connection.manager.findOne(User, new PolarisFindOneOptions(criteria, generateContext()) as any);
            userCommonModel
    ...
    it('delete linked entity, should not return deleted entities(first level), get entity and its linked entity', async () => {
        await connection.manager.delete(Profile, new PolarisCriteria(profileCriteria, generateContext()));
        const userEntity: User | undefined = await connection.manager.findOne(User, new PolarisFindOneOptions({
            ...userCriteria,
            relations: ['profile'],
        }, generateContext()) as any);
    ...
    it('delete entity, should not return deleted entities, doesnt return deleted entity', async () => {
        await connection.manager.delete(Book, new PolarisCriteria(testBookCriteria, generateContext()));
        const book: Book | undefined = await connection.manager.findOne(Book, new PolarisFindOneOptions(testBookCriteria, generateContext()) as any);
    ...
    it('delete entity, soft delete allow is false and return deleted entities true, doesnt return deleted entity', async () => {
        ...
        await connection.manager.delete(Author, new PolarisCriteria(testAuthorCriteria, generateContext()));
        const author: Author | undefined = await connection.manager.findOne(Author, new PolarisFindOneOptions({
    ...
    it(
        ...
        async () => {
            ...
            await connection.manager.delete(Author, new PolarisCriteria(authorWithCascadeCriteria, generateContext()));
            const bookWithCascade: Book | undefined = await connection.manager.findOne(Book, new PolarisFindOneOptions({
                where: {
                    ...bookWithCascadeCriteria.where,
                    ...getAllEntitiesIncludingDeleted.where,
                },
            }, generateContext()) as any);
            const authorWithCascade: Author | undefined = await connection.manager.findOne(Author, new PolarisFindOneOptions({
                where: {
                    ...authorWithCascadeCriteria.where,
                    ...getAllEntitiesIncludingDeleted.where,
                },
    ...
    describe('data version tests', () => {
        it('books are created with data version, get all book for data version 0', async () => {
            const booksInit: Book[] = await connection.manager.find(Book, new PolarisFindManyOptions({},
                generateContext({dataVersion: 0})) as any);
            const booksAfterDataVersion: Book[] = await connection.manager.find(Book, new PolarisFindManyOptions({},
                generateContext({dataVersion: 2})) as any);
        ...
        it('fail save action, data version not progressing', async () => {
            ...
            }, generateContext({realityId: 1})) as any);
    ...
    describe('reality tests', () => {
        it('reality id is supplied in headers', async () => {
            ...
            await connection.manager.save(Book, new PolarisSaveOptions(bookReality1, generateContext({realityId: 1})) as any);
            const book: Book | undefined = await connection.manager.findOne(Book, new PolarisFindOneOptions({}, generateContext({realityId: 1})) as any);
        ...
        it('delete operational entity, linked oper header true and reality id isnt operational, entity not deleted', async () => {
            try {
                await connection.manager.delete(Author, new PolarisCriteria(testAuthorCriteria, generateContext({realityId: 1})));
            } catch (err) {
        ...
        it('save existing entity with different reality id, fail saving', async () => {
            ...
            await connection.getRepository(Book).save(new PolarisSaveOptions(book, generateContext()) as any);
            book.realityId = 1;
            try {
                await connection.manager.save(Book, new PolarisSaveOptions(book, generateContext({realityId: 1})) as any);
            } catch (err) {
    ...
    it('find one with id', async () => {
        ...
        await connection.getRepository(Book).save(new PolarisSaveOptions(book, generateContext()) as any);
        const bookFound: Book | undefined = await connection.manager.findOne(Book, new PolarisFindOneOptions({
            where: {id: book.getId()},
        }, generateContext()) as any);
    ...
    it('count', async () => {
        expect(await connection.manager.count(Book, new PolarisFindManyOptions({}, generateContext()) as any)).toEqual(2);
    });
